...
上下文变量是客户端应用程序在运行时提供的值。 利用这些功能，可根据动态会话级信息（如用户的活动语言、设备类型或应用程序状态）来定位用户，这些标准不属于用户的永久配置文件。

上下文变量与Web、桌面和移动客户端相关。

## 上下文变量的工作方式 {#how-context-works}

在评估功能标记时，您的应用程序会将上下文变量传递到体验转出。 您可以在控制台中定义检查这些值的规则，平台在评估时使用它们来确定用户是否符合条件。

如果您同时在受众规则的&#x200B;**配置文件**&#x200B;和&#x200B;**上下文**&#x200B;部分中定义条件，则所有配置文件条件都将使用AND逻辑与所有上下文条件相结合。
